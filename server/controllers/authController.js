...
export const register = catchAsyncErrors(async (req, res, next) => {
	...
	// const user = await database.query(`INSERT INTO users (name, email, password) VALUES (${name}, ${email}, ${hashedPassword}) RETURNING *`);
});
...
export const login = catchAsyncErrors(async (req, res, next) => {
	...
	sendToken(user.rows[0], 200, "User login successfully.", res);
});
...
export const resetPassword = catchAsyncErrors(async (req, res, next) => {
	...
	const updatedUser = await database.query(`UPDATE users
                                            SET password              = $1,
                                                reset_password_token  = NULL,
                                                reset_password_expire = NULL
                                            WHERE id = $2 RETURNING *`, [hashedPassword, user.rows[0].id]);
